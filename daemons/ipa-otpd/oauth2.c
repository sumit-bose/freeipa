                                    ...
                                    const char *dc_reply, char *rad_reply)
{
    ...
    state_item->oauth2.state.data = calloc(RAD_STATE_LEN, 1);
    if (state_item->oauth2.state.data == NULL) {
        ret = ENOMEM;
        otpd_log_req(child_ctx->item->req, "Failed to allocate state");
        goto done;
    }
    state_item->oauth2.state.length = RAD_STATE_LEN;
   
    ret = getrandom(state_item->oauth2.state.data, RAD_STATE_LEN, 0);
    if (ret != RAD_STATE_LEN) {
        ret = EIO;
        otpd_log_req(child_ctx->item->req, "Failed to get random state");
        goto done;
    } 
...
const char *oauth2_state_to_str(enum oauth2_state oauth2_state)
{
    switch (oauth2_state) {
    case OAUTH2_NO:
...
int oauth2(struct otpd_queue_item **item, enum oauth2_state oauth2_state)
{
    ...
    struct child_ctx *child_ctx;
    char *args[] = {OIDC_CHILD_PATH, NULL, "--issuer-url", NULL, "--client-id", NULL, NULL};
    ...
    if (oauth2_state == OAUTH2_GET_ACCESS_TOKEN) {
        ...
        saved_item = otpd_queue_pop_oauth2_state(&ctx.oauth2_state.states,
                                                 data_state);
        if (saved_item == NULL) {
...
#if 0
        for (saved_item = ctx.oauth2_state.states.head;
                    saved_item != NULL;
                    saved_item = saved_item->next) {

            if (data_state->length == (*item)->oauth2.state.length
                    && memcmp(data_state->data, (*item)->oauth2.state.data,
                              data_state->length) == 0) {
                break;
            }
        }
        ...
        if (otpd_queue_drop_item(&ctx.oauth2_state.states, saved_item) == NULL) {
            otpd_log_req((*item)->req, "Failed to remove item from queue, "
                                       "trying to contine.");
        }
#endif
    }
